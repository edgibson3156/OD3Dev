...
// set colors if item is selected
	...
	if((total=pDoc->m_printSelArr.GetSize()))
	{
		for(i=0; i<total; i++)
		{
			name = pDoc->m_printSelArr.GetAt(i);

			if(!name.CompareNoCase(lvi.pszText))
			{
	...
	{
		if(!name.CompareNoCase(lvi.pszText))
			clrTextSave = pDC->SetTextColor(RGB(255,255,0));
		else
			clrTextSave = pDC->SetTextColor(RGB(0,0,255)/*::GetSysColor(COLOR_HIGHLIGHTTEXT)*/);

		clrBkSave = pDC->SetBkColor(::GetSysColor(COLOR_HIGHLIGHT));

		pDC->FillRect(rcAllLabels, &CBrush(RGB(208,208,245)/*::GetSysColor(COLOR_HIGHLIGHT)*/));
	}
	else
	{
...
COD3Doc* CListViewEx::GetDocument() // non-debug version is inline
{
